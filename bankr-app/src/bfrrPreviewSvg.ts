...
export function bfrrReceiptPreviewDataUri(opts: BfrrPreviewOpts): string {
  ...
  const rawFact = (opts.factory?.trim() || "Unknown").trim();
  const factUpper = esc(clip(rawFact.toUpperCase(), 12));
  const factMixed = esc(clip(rawFact, 14));
  ...
  const yLaunch = showName ? 212 : 210;
  const yPool = showName ? 230 : 228;
  const ySell = showName ? 248 : 246;
  const yFee = showName ? 266 : 264;
...
<text x="118" y="${yLaunch}" font-family="ui-monospace,monospace" font-size="10" fill="#e4e4e7">${factMixed}</text>
<text x="32" y="${yPool}" font-family="ui-monospace,monospace" font-size="10" fill="#71717a" font-weight="600">POOL ID</text>
<text x="118" y="${yPool}" font-family="ui-monospace,monospace" font-size="10" fill="#e4e4e7">${pool}</text>
<text x="32" y="${ySell}" font-family="ui-monospace,monospace" font-size="10" fill="#71717a" font-weight="600">SELLER</text>
<text x="118" y="${ySell}" font-family="ui-monospace,monospace" font-size="10" fill="#e4e4e7">${seller}</text>
<text x="32" y="${yFee}" font-family="ui-monospace,monospace" font-size="10" fill="#71717a" font-weight="600">FEE MGR</text>
<text x="118" y="${yFee}" font-family="ui-monospace,monospace" font-size="10" fill="#e4e4e7">${feeMgr}</text>
