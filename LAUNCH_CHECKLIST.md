...
6. **“Sell + list” orchestration:** If the unified flow includes **`list`**, require a **positive** listing price in ETH before start (`FeeRightsFixedSale` reverts **`ZeroPrice`** for `priceWei == 0`). Do **not** advertise “0 to skip” for that flow; use a separate **“wrap only”** (no list) path if users only want a BFRR without listing. Prefer **`approve(FeeRightsFixedSale, tokenId)`** on BFRR for that listing over **`setApprovalForAll`** when possible — custodial and browser wallets often score single-token approval lower (same `list` contract path: `transferFrom` works with either).
